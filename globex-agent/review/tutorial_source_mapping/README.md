...
## 源码证据约定（2026-09-05 复核）

教程编号只决定文件对应关系，行为结论以当前可执行代码及调用点为准。源码注释、教程目标、配置项存在均不单独证明能力生效。流程箭头仅用于确有调用/执行顺序的节点；组装关系和未来方案应另行说明。链接存在、行号未超界只说明目标可定位，不证明链路正确，也不证明运行成功。

本轮已纠正事件名、检索字段与分支、偏好写入方法、缓存顺序、启动顺序、护栏时机和观测关系；这是针对主链的静态复核，39 篇仍不能视为已逐项完成运行验证。删除旧清单前需要核对其独有内容是否已迁移，不能仅凭标题重叠判断冗余。
